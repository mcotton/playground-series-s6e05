...
- **Tuning CatBoost did not save the 3-way blend — CatBoost is a confirmed dead end (Exp 22).** Optuna made the CB single model *worse* (OOF 0.92997 vs untuned 0.93130 — the search overfit, picking depth=10, its max bound). The equal-weight 3-way blend then scored LB 0.95166, a dead tie with the 2-way champion (0.95168, Δ−0.00002). Even tuned, CatBoost is too weak/correlated here to beat XGB+LGBM; it only matches it. **Stop trying to add CatBoost.**
- **Equal weights beat the simplex OOF-weight optimizer on LB (Exp 22 vs 21).** Same three models, two weightings: optimized (sub_28, .52/.21/.27) → LB 0.95144 at OOF 0.93645; equal 1/3-each (sub_29) → LB 0.95166 at OOF 0.93610. The equal-weight blend won LB by +0.00022 *despite lower OOF* — the optimizer chases OOF, skews weight onto the weak/diverse model, and gives it back on the test set. **Default to equal weights for blends on this comp; don't trust the simplex grid search.**
...
- **Priority order for remaining experiments:**
  ...
  4. ~~**CatBoost as a third model.**~~ **Done (Exp 20-22). Dead end.** Diversity is real (corr 0.96), but tuned or not, CB only ties the 2-way blend on LB. Dropped.
  5. ~~**Tune CatBoost with optuna.**~~ **Done (Exp 22).** Tuning made the single model *worse* (overfit to depth=10); equal-weight 3-way tied the 2-way (LB 0.95166 vs 0.95168). Not pursuing further.
  6. **Seed-bagging XGB** — refit the tuned XGB at 5 different `random_state` values and average. ~30 min on GPU, almost guaranteed small uplift, very low risk. **Now the highest-leverage next step.**
  7. **Pseudo-labeling** — train on confident test predictions added to train. Higher risk, +0.001-0.003 typical.
...
- [x] CatBoost (Exp 20-22) — ordered target stats + oblivious trees give corr ~0.96 with the GBMs (vs 0.99 between them). Diversity is real, but **even after its own optuna run (Exp 22) the single-OOF is still ~0.005 below the tuned GBMs and the 3-way blend only ties the 2-way on LB.** Confirmed dead end; not used in the live submission.
